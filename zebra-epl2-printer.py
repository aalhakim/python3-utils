...
class EplCommand(object):
    def __init__(self, printer, width_mm, length_mm, columns=1, label_gap_mm=0, x_offset_mm=0.0, y_offset_mm=0.0):
        ...
        self.ribbon_width_dot = (columns * int(width_mm / DOT_MM)) + ((columns-1) * self.array_gap_dot)
    ...
    def set_codepage(self, lang='1'):
        ...
        """
        assert(0<=int(lang)<=15), "Invalid_setting (lang of {}): 18 <= lang <= 240".format(lang)
    ...
    def set_form_length(self, gap_length):
        ...
        """
        assert(16 <= gap_length <= 240), "Invalid setting (gap_length of {}): 16 <= gap_length <= 240".format(gap_length)
    ...
    def encode_barcode_c128(self, string, start_x=0, start_y=0, height=10):
        ...
        """
        start_x = int(start_x) + ((self.label_width_dot + self.array_gap_dot) * self.active_column) + self.x_offset
        if start_x < 0:
            start_x = 0
        ...
        barcode = (
            'B{},{},0,1,{},{},{},N,"{}"\n'
            .format(start_x, start_y, narrow_bar, wide_bar, height, string)
            )
        self.command += barcode

    def encode_text(self, string, start_x, start_y, font_size=3, expand_x=1, expand_y=1, invert=False):
        """
        ...
        """
        assert(1<=font_size<=5), "Invalid setting (font_size of {}): 1 <= font_size <= 5".format(font_size)
        assert(invert==True or invert==False), "Invalid setting: (invert of {}): invert can only be True or False".format(invert)
        if invert:
            invert = "R"
        else:
            invert = "N"
        ...
        text = (
            'A{},{},0,{},{},{},{},"{}"\n'
            .format(start_x, start_y, font_size, expand_x, expand_y, invert, string)
            )
        self.command += text
...
class Barcode(object):
    ...
    def width(self, unit="DOT"):
        if unit.upper() == "DOT":
            ...
            raise RuntimeError("ERROR: Unknown unit used: {}. 'DOT' or 'MM' only allowed.".format(unit))
...
class Printer(object):
    def __init__(self, printer, width, length, columns, gap_size, x_offset_mm=0.0, y_offset_mm=0.0):
        self.printer = printer
        self.label_width = width
        self.label_length = length
        self.columns = columns
        self.commander = EplCommand(printer, width, length, columns, gap_size, x_offset_mm, y_offset_mm)
    ...
    def print_title(self, text_list, override_exit=False):
        ...
        for i, string in enumerate(text_list, start=1):
            ...
            self.commander.encode_text(text.text, start_x=text.start_pos, start_y=5, font_size=text.font_size)
    ...
    def print_text(self, text_list, font_size=3, centre=False, override_exit=False):
        ...
        for i, string in enumerate(text_list, start=1):
            ...
            self.commander.encode_text(text.text, start_x=start_x_pos, start_y=start_y_pos, font_size=text.font_size)
        ...
        # If next_label has not been issued 'self.columns' times, then
        # force the printer to move to the next row.
        if not override_exit and new_row is False: #i % (line_limit * self.columns) != 0:
            self.commander.encode_print_label()
            self.commander.send_command()

    def print_barcode(self, text, quantity=1, invert=False, override_exit=False, bc_height_mm=None):
        ...
        for font_size in reversed(range(1,4)):
            _text = Text(barcode.text, self.label_width, font_size=font_size, x_offset_dot=2)
        ...
        # of the text height
        text_start_pos = bc_height_dot + int((4-font_size) * 0.2 * _text.height("DOT"))
        for n in range(1, quantity+1):
            self.commander.encode_barcode_c128(barcode.text, start_x=barcode.start_pos, start_y=0, height=bc_height_dot)
            self.commander.encode_text(_text.text, start_x=_text.start_pos, start_y=text_start_pos, font_size=_text.font_size, invert=invert)
            self.commander.next_label()
...
if __name__ == '__main__':
    ...
    ####################################################################
    zebraPrinter = Printer(zprinter, width, height+height_adjust_mm, columns, label_gap_mm, x_offset_mm, y_offset_mm)

    # An example of a large text barcode.
    zebraPrinter.print_barcode("S/N:00000001", 1, bc_height_mm=barcode_height_mm, override_exit=True)

    # An example of a medium text barcode.
    zebraPrinter.print_barcode("S/N:0000000000001", 1, bc_height_mm=barcode_height_mm, override_exit=True)

    # An example of a small text barcode.
    zebraPrinter.print_barcode("S/N:000000000000000001", 1, bc_height_mm=barcode_height_mm, override_exit=True)
    ...
    zebraPrinter.print_title("BEGIN", override_exit=True) # The necessity for the final override exit is a bug.
