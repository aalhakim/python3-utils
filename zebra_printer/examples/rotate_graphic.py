...
"""
Example code to print out a QR Code using zprinter.py
"""
...
if __name__ == "__main__":
    ...
    for n in range(quantity):
        ...
        if last_label is True and n % SCROLL_COLUMNS != max_column:
            zebraPrinter.print_qr_with_text(
                serial_number, quantity=1, scale=scale, override_exit=False
            )

        elif last_label is True:
            zebraPrinter.print_qr_with_text(
                serial_number, quantity=1, scale=scale, override_exit=True
            )

        else:
            zebraPrinter.print_qr(
                serial_number, quantity=1, scale=scale, override_exit=True
            )
